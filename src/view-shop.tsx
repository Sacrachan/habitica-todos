...
type FilterValue =
  | "all" | "affordable" | "market" | "reward"
  | "weapon" | "armor" | "head" | "shield"
  | "headAccessory" | "eyewear" | "back" | "body";
...
const GEAR_TYPE_LABEL: Record<string, string> = {
  weapon: "Weapon", armor: "Armor", head: "Helmet", shield: "Off-Hand",
  headAccessory: "Head Accessory", eyewear: "Eyewear", back: "Back Accessory", body: "Body Accessory",
};
...
export default function Command() {
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true);
    try {
      const [rewards, userData, content] = await Promise.all([
        getTasks("rewards"),
        getUser(),
        getContent(),
      ]);
      ...
      const shopItems: ShopItem[] = [
        ...(userData.stats.hp < (userData.stats.maxHealth ?? 50)
          ? [{ id: "health_potion", text: "Health Potion", notes: "Restores 15 Health.",
               value: 25, type: "market" as const, icon: Icon.Heart }]
          : []),
        ...(userData.stats.lvl >= 10
          ? [{ id: "enchanted_armoire", text: "Enchanted Armoire", notes: "Get either gear, food, or XP!",
               value: 100, type: "market" as const, icon: Icon.Box,
               imageUrl: `${GEAR_ASSET_BASE}/shop_armoire.png` }]
          : []),
        ...buildGearItems(userData, content),
        ...rewards.map((r) => ({ id: r.id, text: r.text, notes: r.notes,
                                  value: r.value, type: "reward" as const })),
      ];
  ...
  async function handleBuy(item: ShopItem) {
    if (user && user.stats.gp < item.value) {
      await showToast({ style: Toast.Style.Failure, title: "Not enough gold!",
      ...
      <List.Item.Detail
        ...
        metadata={
          <List.Item.Detail.Metadata>
            ...
            {item.notes
              ? <List.Item.Detail.Metadata.Label title="Description" text={item.notes} />
              : null}
            ...
            />
            {categoryLabel
              ? <List.Item.Detail.Metadata.Label title="Category" text={categoryLabel} />
              : null}
          </List.Item.Detail.Metadata>
        }
      />
  ...
  const filterDropdown = (
    <List.Dropdown tooltip="Filter items" value={filter}
    ...
    >
      {showMarket && (
        <List.Section title="Market" subtitle={goldLabel}>
          {filteredItems.filter((i) => i.type === "market").map((item) => (
            <List.Item
              key={item.id}
              title={item.text}
      ...
      {showGear && GEAR_TYPE_ORDER.map((slot) => {
      ...
      {showRewards && (
        <List.Section title="Custom Rewards" subtitle={goldLabel}>
          {filteredItems.filter((i) => i.type === "reward").map((item) => (
            <List.Item
              key={item.id}
              title={item.text}
              subtitle={`${item.value} GP`}
