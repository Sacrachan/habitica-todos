...
export default function Command() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      getAvatarSvg(data)
        .then((uri) => { if (avatarGenRef.current === gen) setAvatarUri(uri); })
  ...
  if (quest?.key) {
    ...
    if (quest.progress?.collect)
      questMarkdown += `**Items to Collect:**\n` +
        Object.entries(quest.progress.collect).map(([k, v]) => `- ${k}: ${v}`).join("\n");
  }
    ...
    <Detail
      ...
      metadata={
        <Detail.Metadata>
          ...
          <Detail.Metadata.Label title="Health"     text={`${(stats?.hp ?? 0).toFixed(1)} / ${stats?.maxHealth ?? 50}`} icon={{ source: Icon.Heart,     tintColor: Color.Red    }} />
          <Detail.Metadata.Label title="Mana"       text={(stats?.mp ?? 0).toFixed(1)}                              icon={{ source: Icon.Star,      tintColor: Color.Blue   }} />
          <Detail.Metadata.Label title="Experience" text={`${(stats?.exp ?? 0).toFixed(1)} / ${stats?.toNextLevel ?? "?"}`} icon={{ source: Icon.ChevronUp, tintColor: Color.Yellow }} />
          <Detail.Metadata.Label title="Gold"       text={(stats?.gp ?? 0).toFixed(2)}                             icon={{ source: Icon.Coins,     tintColor: Color.Yellow }} />
          {quest?.key && (
            <Detail.Metadata.TagList title="Quest Status">
              <Detail.Metadata.TagList.Item text={quest.active ? "Active" : "Pending"} color={quest.active ? Color.Green : Color.Blue} />
        ...
        <ActionPanel>
          ...
          {quest?.key && (
            <ActionPanel.Section title="Quest Actions">
              {!quest.active && <Action title="Accept Quest"        icon={Icon.CheckCircle} onAction={() => handleQuestAction("accept")} />}
              {quest.active  && <Action title="Force Complete Quest" icon={Icon.Stars}       onAction={() => handleQuestAction("force-complete")} />}
              <Action title="Abort Quest" icon={Icon.XMarkCircle} style={Action.Style.Destructive} onAction={() => handleQuestAction("abort")} />
            </ActionPanel.Section>
          )}
          <ActionPanel.Section>
            <Action.OpenInBrowser title="Open Habitica" url="https://habitica.com" shortcut={{ modifiers: ["cmd"], key: "o" }} />
          </ActionPanel.Section>
        </ActionPanel>
      }
    />
